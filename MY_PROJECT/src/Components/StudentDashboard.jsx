...
const StudentDashboard = ({ user, onNavigate }) => {
  ...
  const userMilestones = MOCK_MILESTONES.slice(0, 3);
  const groupMembers = MOCK_USERS.filter(u => u.role === 'student' && u.id !== user.id).slice(0, 3);
    ...
    <div className="student-dashboard">
      ...
      <div className="section">
        ...
        <div className="task-list">
          {userTasks.length > 0 ? userTasks.map(task => <TaskCard key={task.id} task={task} />) : <p className="empty-state">No tasks assigned yet</p>}
        </div>
      </div>
      <div className="section">
          ...
          {userMilestones.map(milestone => (
            <div key={milestone.id} className="milestone-item">
              <div className="milestone-header">
                ...
                <span className={`status-badge status-${milestone.status}`}>{milestone.status}</span>
              </div>
              <ProgressBar progress={milestone.progress} label={`Progress: ${milestone.progress}%`} />
